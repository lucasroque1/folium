  ...
  return (
    <div style={card}>
      ...
      <div style={{ padding: 10 }}>
        ...
        <p style={{ margin: '6px 0', fontSize: 13, color: '#444' }}>{(book.description || '').slice(0, 120)}{(book.description || '').length > 120 ? '...' : ''}</p>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Link to={`/books/${id}`} style={{ textDecoration: 'none', color: '#1a73e8' }}>Details</Link>
          <span style={{ fontSize: 12, padding: '2px 6px', border: '1px solid #ddd', borderRadius: 4 }}>{book.type || 'book'}</span>
        </div>
      </div>
    </div>
  );
